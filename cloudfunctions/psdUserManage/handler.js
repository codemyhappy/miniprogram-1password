...
module.exports.handlerInit = function(collectionName,{event, context}=option){
  ...
  async function getMineInfo(){
    ...
    // 取第一个user
    const userinfo = rst.data && rst.data.dataList && rst.data.dataList.length>=1 ? rst.data.dataList[0] : null
  ...
  /**
   * 返回isValid
   * @param {string} encryptedPassword 加密串
   */
  async function verifyPassword(encryptedPassword){
    if(!encryptedPassword) return newResponse({isValid:false})
    ...
    console.log('verifyPassword',rst)
    // 取第一个
    const isValid = rst.data && rst.data.dataList && rst.data.dataList.length>=1 ? true : false
  ...
  return {
    getMineInfo,verifyPassword,
    add,updateById,deleteOne,get,getOne,getWXContext
  }
}